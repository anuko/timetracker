...
class ttFavReportHelper {
  ...
  static function insertReport($fields) {
    ...
    $sql = "insert into tt_fav_reports (name, user_id, client_id, cf_1_option_id, project_id, task_id,
      ...
      show_task, show_end, show_note, show_billable, show_clientnumber, show_custom_field_1,
      ...
      values(" .
            ...
            $fields['chtask'] . ", " . $fields['chfinish'] . ", " . $fields['chnote'] . ", " . $fields['chbillable'] . ", " . $fields['chclientnumber'] . ", " . $fields['chcf_1'] . ", " .
  ...
  function updateReport($fields) {
    ...
    $sql = "update tt_fav_reports set " .
            ...
            "show_billable = " . $fields['chbillable'] . ", " .
            "show_clientnumber = " . $fields['chclientnumber'] . ", " .
  ...
  static function saveReport($user_id, $bean) {
    ...
    if (!$bean->getAttribute('chclient'))
      $bean->setAttribute('chclient', 0);
    if (!$bean->getAttribute('chinvoice'))
      $bean->setAttribute('chinvoice', 0);
    if (!$bean->getAttribute('chproject'))
      $bean->setAttribute('chproject', 0);
    if (!$bean->getAttribute('chstart'))
      $bean->setAttribute('chstart', 0);
    if (!$bean->getAttribute('chduration'))
      $bean->setAttribute('chduration', 0);
    if (!$bean->getAttribute('chcost'))
      $bean->setAttribute('chcost', 0);
    if (!$bean->getAttribute('chtask'))
      $bean->setAttribute('chtask', 0);
    if (!$bean->getAttribute('chfinish'))
      $bean->setAttribute('chfinish', 0);
    if (!$bean->getAttribute('chnote'))
      $bean->setAttribute('chnote', 0);
    if (!$bean->getAttribute('chbillable'))
      $bean->setAttribute('chbillable', 0);
    if (!$bean->getAttribute('chclientnumber'))
      $bean->setAttribute('chclientnumber', 0);
    if (!$bean->getAttribute('chcf_1'))
      $bean->setAttribute('chcf_1', 0);
    if (!$bean->getAttribute('chtotalsonly'))
      $bean->setAttribute('chtotalsonly', 0);
    ...
    $fields = array(
        ...
        'chbillable' => $bean->getAttribute('chbillable'),
        'chclientnumber' => $bean->getAttribute('chclientnumber'),
  ...
  static function loadReport($user_id, &$bean) {
    ...
    if ($val) {
      ...
      $bean->setAttribute('chbillable', $val['show_billable']);
      $bean->setAttribute('chclientnumber', $val['show_clientnumber']);
