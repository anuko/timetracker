...
class ttTaskHelper {
  ...
  static function getTask($id)
  {
    ...
    $sql = "select id, name, description, allow_empty_duration, status from tt_tasks
 ...
 // insert function inserts a new task into database.
  static function insert($fields)
  {
    ...
    $allow_empty_duration = (int) $fields["allow_empty_duration"];
    $status = $fields['status'];

    $sql = "insert into tt_tasks (team_id, name, description, allow_empty_duration, status)
      values ($team_id, ".$mdb2->quote($name).", ".$mdb2->quote($description).", ".$allow_empty_duration.", ".$mdb2->quote($status).")";
    $affected = $mdb2->exec($sql);
    $last_id = 0;
    if (is_a($affected, 'PEAR_Error'))
      return false;
 ...
 // update function updates a task in database.
  static function update($fields)
  {
    ...
    $sql = "update tt_tasks set name = ".$mdb2->quote($name).", description = ".$mdb2->quote($description).
      ", allow_empty_duration = ".$allow_empty_duration.", status = $status where id = $task_id";
    $affected = $mdb2->exec($sql);
    if (is_a($affected, 'PEAR_Error'))
