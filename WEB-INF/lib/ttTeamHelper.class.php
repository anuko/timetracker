...
class ttTeamHelper {
  ...
  static function update($team_id, $fields)
  {
    ...
    if (isset($fields['working_hours'])) $work_hours_part = ', workday_hours = '.$mdb2->quote($fields['working_hours']);
    ...
    $sql = "update tt_teams set $name_part $addr_part $currency_part $lang_part $decimal_mark_part
      ...
      $plugins_part $lock_spec_part $work_hours_part where id = $team_id";
    $affected = $mdb2->exec($sql);
    if (is_a($affected, 'PEAR_Error')) return false;
